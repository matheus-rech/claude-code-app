...
export default function DiffViewer({
  ...
  if (loading) {
    return (
      <div className={`flex items-center justify-center p-8 ${className}`}>
        <div className="text-subtext-0">Processing diff...</div>
      </div>
    )
  }
  ...
  if (!processedFiles.length) {
    return (
      <div className={`flex items-center justify-center p-8 ${className}`}>
        <div className="text-subtext-0">No changes to display</div>
      </div>
    )
  }
...
function DiffFile({
  file,
  staged,
}: { file: ProcessedDiffFile; staged?: boolean }) {
  const getFileStatus = () => {
  ...
  return (
    <div className="border border-surface-1 rounded-lg overflow-hidden mb-4">
      {/* File header */}
      <div className="bg-surface-0 px-4 py-2 border-b border-surface-1">
        <div className="flex items-center justify-between">
          ...
          <div className="text-sm text-subtext-1">
            <span className="text-green">+{file.additions || 0}</span>
            <span className="mx-1">/</span>
            <span className="text-red">-{file.deletions || 0}</span>
          </div>
        </div>
      </div>
      ...
      {/* Diff content */}
      <div
        className="bg-base overflow-y-auto overflow-x-auto"
            ...
            {file.chunks.map((chunk, chunkIndex) => (
              <React.Fragment key={chunkIndex}>
                ...
                <tr>
                  <td
                    colSpan={3}
                    className="px-4 py-1 bg-surface-0 text-subtext-0 text-xs"
                  >
                    {chunk.content}
                  </td>
                </tr>
...
function DiffLine({ change }: { change: ProcessedChange }) {
  ...
  return (
    <tr className={getBackgroundColor()}>
      {/* Old line number */}
      <td className="px-2 py-0 text-right text-subtext-0 select-none w-12 min-w-12">
        {oldLineNumber}
      </td>
      ...
      {/* Code content */}
      <td className="px-2 py-0 text-text">
            ...
            <span
              ...
              style={{
                ...
                backgroundColor: token.isWordDiff
                  ? token.diffType === "added"
                    ? "rgba(34, 197, 94, 0.3)"
                    : token.diffType === "removed"
                      ? "rgba(239, 68, 68, 0.3)"
                      : undefined
                  : undefined,
              }}
